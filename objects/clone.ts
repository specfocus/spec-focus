const clone = (src: any, seen: Map<unknown, unknown>) => {
  ...
  if (src.nodeType && 'cloneNode' in src) {
    ...
    copy = new Map();
    seen.set(src, copy);
    for (const [k, v] of Array.from(src.entries())) copy.set(k, clone(v, seen));
  } else if (src instanceof Set) {
    // Set
    copy = new Set();
    seen.set(src, copy);
    for (const v of Array.from(src)) copy.add(clone(v, seen));
...
export default (src?: unknown) => clone(src, new Map());