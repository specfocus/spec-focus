...
schemas comes with robust typescript support! However, not everything can be statically
typed safely, but for most cases it's "Good Enough".

Note that schema actually produce _two_ different types: the result of casting an input, and the value after validation.
...
extend the abstract `AnythingSchema` class.

### Special note about MixedSchema and AnythingSchema

As far as typescript is concerned, `mixed` schema inherit from `AnythingSchema` like other schema; all other schema do **not** extend `MixedSchema`. **In actuality** Mixed is an alias for AnythingSchema, meaning `addMethod(mixed)` will add a new method to all schema.

This means that type extensions to `mixed` should generally be put on `AnythingSchema` if
