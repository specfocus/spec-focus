...
import { isNaN, isNumberOrString } from './unary';
...
class GlobalDomainRuleMap {
  ...
  Array: GlobalDomainRule<unknown[]> = {test: isArray, name: nameOf2<GlobalDomainRuleMap>((o) => o.Array)};
  Boolean: GlobalDomainRule<boolean> = {test: isBoolean, name: nameOf2<GlobalDomainRuleMap>((o) => o.Boolean)};
  EmailAddress: GlobalDomainRule<EmailAddress> = {test: isEmailAddress, name: nameOf2<GlobalDomainRuleMap>((o) => o.EmailAddress)};
  Guid: GlobalDomainRule<Guid> = {test: isGuid, name: nameOf2<GlobalDomainRuleMap>((o) => o.Guid)};
  Intenger: GlobalDomainRule<Integer> = {test: isInteger, name: nameOf2<GlobalDomainRuleMap>((o) => o.Intenger)};
  LowerCase: GlobalDomainRule<LowerCase> = {test: isLowerCase, name: nameOf2<GlobalDomainRuleMap>((o) => o.LowerCase)};
  NaN: GlobalDomainRule<number> = {test: isNaN, name: nameOf2<GlobalDomainRuleMap>((o) => o.NaN)};
  ...
  Nil: GlobalDomainRule<Nil> = {test: isNil, name: nameOf2<GlobalDomainRuleMap>((o) => o.Nil)};
  ...
  Null: GlobalDomainRule<null> = {test: isNull, name: nameOf2<GlobalDomainRuleMap>((o) => o.Null)};
  Number: GlobalDomainRule<number> = {test: isNumber, name: nameOf2<GlobalDomainRuleMap>((o) => o.Number)};
  ...
  String: GlobalDomainRule<string> = {test: isString, name: nameOf2<GlobalDomainRuleMap>((o) => o.String)};
  UpperCase: GlobalDomainRule<UpperCase> = {test: isUpperCase, name: nameOf2<GlobalDomainRuleMap>((o) => o.UpperCase)};
}

const _map = Object.freeze(new GlobalDomainRuleMap());
const _keys = Object.freeze(Object.keys(_map).sort()) as readonly GlobalDomainName[];
const _values = Object.freeze(_keys.map((k) => _map[k]));

export function ruleFor<T>(ruleFunction: (obj: GlobalDomainRuleMap) => GlobalDomainRule<T>): GlobalDomainRule<T> {
...
export class GlobalDomain {
  ...
  static test = (name: GlobalDomainName, val: unknown): boolean => _map[name]?.test(val);
  static map = _map;
  static all = _values;
}
